<script lang="ts">
    ...
    import {
        ...
        type QpsHistoryResponse,
    } from "../lib/api";
    import { formatUptime, formatNumber } from "../lib/mock";
    ...
    $: upstreamSummary = {
        ...
        down: upstreams.filter(
            (u) => u.status === "down" || u.status === "unhealthy",
        ).length,
    };
    ...
    async function fetchData() {
        try {
            const [overviewRes, upstreamRes, qpsRes] = await Promise.all([
...
<div class="space-y-6">
    ...
    {#if error}
        <div
            class="card p-4 bg-red-500/10 border border-red-500/20 text-red-400"
        >
            <p>Error: {error}</p>
            <button
