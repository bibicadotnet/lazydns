...
<div class="overflow-y-auto space-y-2" style="max-height: {maxHeight}">
    ...
    >
      <div class="flex items-start justify-between">
        <div class="flex items-start gap-3">
          <span class="text-xl">{eventTypeLabels[event.event_type].icon}</span>
          <div>
            <div class="flex items-center gap-2 mb-1">
              <span
                class="font-medium {eventTypeLabels[event.event_type].color}"
              >
                {eventTypeLabels[event.event_type].label}
              </span>
              <span
                class="text-xs {$darkMode ? 'text-gray-500' : 'text-gray-700'}"
              >
  ...
  {:else}
    <div
      class="text-center py-12 {$darkMode ? 'text-gray-500' : 'text-gray-400'}"
    >
      <svg
