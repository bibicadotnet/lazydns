                ...
                {#each upstreams as upstream}
                    <tr>
                        <td>
                            <div class="flex items-center gap-2">
                                <div
                                    ...
                                    class:bg-green-400={upstream.status ===
                                        "healthy"}
                                    class:bg-yellow-400={upstream.status ===
                                        "degraded"}
                                    class:bg-red-400={upstream.status ===
                                        "down"}
                                ></div>
                                <span
                                    class="text-xs font-medium uppercase"
                                    class:text-green-400={upstream.status ===
                                        "healthy"}
                                    class:text-yellow-400={upstream.status ===
                                        "degraded"}
                                    class:text-red-400={upstream.status ===
                                        "down"}
                                >
                                    {upstream.status}
                                </span>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                >
                                    <div
                                        ...
                                        class:bg-green-500={upstream.avg_latency_ms <
                                            50}
                                        class:bg-yellow-500={upstream.avg_latency_ms >=
                                            50 && upstream.avg_latency_ms < 100}
                                        class:bg-red-500={upstream.avg_latency_ms >=
                                            100}
                                        style="width: {Math.min(
                                            100,
                                            upstream.avg_latency_ms / 2,
                                        )}%"
                                    ></div>
                                </div>
                                <span
                                    class="text-sm font-medium"
                                    class:text-green-400={upstream.avg_latency_ms <
                                        50}
                                    class:text-yellow-400={upstream.avg_latency_ms >=
                                        50 && upstream.avg_latency_ms < 100}
                                    class:text-red-400={upstream.avg_latency_ms >=
                                        100}
                                >
                        ...
                        <td>
                            <span
                                class="font-medium"
                                class:text-green-400={upstream.success_rate >=
                                    99}
